...
def mostrar_resultado_multi():
    n1_ = float(n1.get())
    n2_ = float(n2.get())
    multi  = n1_ *  n2_
    resultado.config(text=multi) 
...
n2_label =  tk.Label(root, text='N1', font=('arial',12))
...
btn_sub.grid(row=4, column= 1, pady=5)


btn_sub =  tk.Button(root, text= '*', font=('arial',10), command=mostrar_resultado_multi)
btn_sub.grid(row=3, column= 1, pady=5)


btn_sub =  tk.Button(root, text= '/', font=('arial',10), command=mostrar_resultado_div)
